...
if game.PlaceId ~= 13864667823 then
	...
	-- Free Gamepasses (LOBBY)
		...
		local Window = OrionLib:MakeWindow({
			Name = "Beluga X Break In 2 Early Access",
			HidePremium = false,
			SaveConfig = false,
			ConfigFolder = "BXBreakIn",
			IntroText = "Beluga X is loading."
		})
		local Tab = Window:MakeTab({
			Name = "lobby not supported",
			Icon = "rbxassetid://4483345998",
			PremiumOnly = false
		})
		OrionLib:Init()
	else
		game:GetService("Players").LocalPlayer:Kick("game isnt supported")
	end
else
    	-- Floating Part
